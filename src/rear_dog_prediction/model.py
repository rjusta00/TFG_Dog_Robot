...
class RearDogGRUPredictor(nn.Module):
    def __init__(
        self,
        input_size: int,
        hidden_size: int,
...
def build_windows(
    ...
    for video_id, video_rows in rows_by_video.items():
        ...
        for row in video_rows:
            ...
            segment_key = f"{video_id}:{segment_id}"
        ...
        for segment_rows in segments.values():
            ...
            for row in segment_rows:
                if previous_row is not None:
                    ...
                    previous_frame = int(previous_row["frame"])
                    same_track = (
                        row["dog_track_id"]
                        == previous_row["dog_track_id"]
                    )

                    if current_frame != previous_frame + 1 or not same_track:
                        previous_row = None
            ...
            for end_index in range(
                history_length - 1,
                len(segment_rows) - prediction_offset_frames,
            ):
                start_index = end_index - history_length + 1
                future_index = end_index + prediction_offset_frames

                history_rows = segment_rows[
                    start_index:end_index + 1
                ]

                future_row = segment_rows[
                    future_index
                ]
                ...
                for index in range(1, len(history_rows)):
                    previous_frame = int(history_rows[index - 1]["frame"])
                    current_frame = int(history_rows[index]["frame"])
                ...
                if int(future_row["frame"]) != int(segment_rows[end_index]["frame"]) + prediction_offset_frames:
                    continue

                feature_window = np.stack(
                    segment_features[start_index:end_index + 1],
                    axis=0,
                )
                ...
                all_metadata.append(
                    WindowMetadata(
                        video_id=video_id,
                        current_frame=int(current_row["frame"]),
                        future_frame=int(future_row["frame"]),
                        current_row=current_row,
                        future_row=future_row,
                    )
                )
    ...
    return (
        np.stack(all_features).astype(np.float32),
...
def normalize_features(
    ...
    if features.size == 0:
        raise ValueError(
            "Cannot normalize an empty feature array."
        )
